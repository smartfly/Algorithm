package leetcode;

/**
 * Created with IntelliJ IDEA.
 *
 * @author: hutao01
 * Date: 2018/1/17
 * Time: 11:20
 * Description: median two sort arrays
 * To change this template use File | Settings | File Templates | Includes | File Header
 */
public class MedianTwoSortedArrays {

    public double findMedianSortedArrays(int[] nums1, int[] nums2) {

        int m = nums1.length, n = nums2.length;
        // left half of the combined median
        int left = (m + n + 1) / 2;
        // right half of the combined median
        int right = (m + n + 2) / 2;

        // If the num1.length + num2.length is odd, the 2 function will return the same number
        // Else if num1.length + num2.length is even, the 2 function will return the left number
        // and right number that make up a median
        return (getKthElement(nums1, 0, nums2, 0, left) + getKthElement(nums1, 0, nums2, 0, right))/2.0;
    }

    /**
     * This function finds the Kth element in num1 + num2
     * @param nums1
     * @param start1
     * @param nums2
     * @param start2
     * @param k
     * @return
     */
    private double getKthElement(int[] nums1, int start1, int[] nums2, int start2, int k){
        // If nums1 is exhausted, return kth number in nums2
        if(start1 > nums1.length -1){
            return nums2[start2 + k - 1];
        }

        // If num2 is exhausted, return kth number in nums1
        if(start2 > nums2.length - 1){
            return nums1[start1 + k - 1];
        }

        // If k == 1, return the first number
        // Since nums1 and nums2 is sorted, the smaller one among the start point of nums1 and nums2 is the first one
        if(k == 1){
            return Math.min(nums1[start1], nums2[start2]);
        }

        int mid1 = Integer.MAX_VALUE;
        int mid2 = Integer.MAX_VALUE;
        if (start1 + k/2 -1 < nums1.length){
            mid1 = nums1[start1 + k/2 -1];
        }

        if (start2 + k/2 - 1 < nums2.length){
            mid2 = nums2[start2 + k/2 -1];
        }

        // Throw away half of the array from nums1 or nums2. And cut k in half
        if (mid1 < mid2){
            return getKthElement(nums1, start1 + k/2, nums2, start2, k - k/2);
        } else {
            return getKthElement(nums1, start1, nums2, start2 + k/2, k - k/2);
        }

    }

}
